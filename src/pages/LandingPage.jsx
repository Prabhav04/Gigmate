...
// ─── New Setlist Modal ────────────────────────────────────────────────────────
const NewSetlistModal = ({ onClose, onCreate }) => {
        ...
        >
            ...
            >
                ...
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white leading-tight">New Setlist</h2>
                        <p className="text-sm text-slate-400">Give it a name to get started</p>
                    </div>
                </div>
                ...
                <p className="text-[11px] text-slate-600 mb-6">
                    This becomes your session ID. Share it with your band after creating.
                </p>
...
const LandingPage = () => {
    ...
    // Called by the modal when user submits a name
    const createSetlist = (name) => {
        const id = generateSetlistId(name);
        saveSessionToHistory(id, name);
        // Pass the human name through navigation state so SessionSpace can show it
        navigate(`/session/${id}`, { state: { sessionName: name } });
    };
        ...
        <>
            ...
            <div className="min-h-[90vh] flex flex-col items-center justify-start pt-10 md:pt-20 space-y-12 pb-10">
                ...
                <div className="flex flex-col space-y-4 animate-fade-in-up text-center items-center">
                    <svg width="280" height="59" viewBox="0 0 354 75" fill="none" xmlns="http://www.w3.org/2000/svg"
                ...
                {/* ── Bottom section: Recent Gigs + Action Cards ───────── */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl px-4">
                        ...
                        {savedSessions.length > 0 ? (
                            <div className="space-y-3 overflow-y-auto pr-2 pb-4 flex-1">
                                {savedSessions.map((session) => (
                                    <div
                                        key={session.id}
                                        onClick={() => navigate(`/session/${session.id}`, { state: { sessionName: session.name } })}
                                    ...
                                    >
                                        <div className="flex items-center gap-4 min-w-0">
                                            <div className="w-10 h-10 shrink-0 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 font-bold group-hover:bg-primary/20 group-hover:text-primary transition-colors">
                                                {session.name.charAt(0).toUpperCase()}
                                            </div>
                                            <div className="min-w-0">
                                                <div className="font-bold text-white group-hover:text-primary transition-colors truncate">{session.name}</div>
                                                <div className="text-[10px] text-slate-500 font-mono truncate">
                                                    {session.id} · {new Date(session.date).toLocaleDateString()}
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="text-center py-12 text-slate-600 border-2 border-dashed border-slate-800 rounded-xl">
                                No recent gigs found. Create a setlist above!
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div
                            ...
                            className="group relative p-6 rounded-2xl cursor-pointer transition-all duration-300 hover:-translate-y-1 overflow-hidden"
                            style={{
                                background: 'linear-gradient(135deg, rgba(124,58,237,0.12) 0%, rgba(79,70,229,0.12) 100%)',
                                border: '1px solid rgba(124,58,237,0.3)',
                                boxShadow: '0 0 0 0 rgba(124,58,237,0)',
                            }}
                            onMouseEnter={e => e.currentTarget.style.boxShadow = '0 0 32px rgba(124,58,237,0.2)'}
                            onMouseLeave={e => e.currentTarget.style.boxShadow = '0 0 0 0 rgba(124,58,237,0)'}
                        >
                            {/* Glow blob */}
                            <div className="absolute -top-4 -right-4 w-24 h-24 rounded-full opacity-20 group-hover:opacity-40 transition-opacity"
                                style={{ background: 'radial-gradient(circle, #7c3aed, transparent 70%)' }} />
                            <div className="relative z-10">
                                <div className="flex items-center gap-2 mb-2">
                                    <ListMusic size={20} className="text-primary" />
                                    <h2 className="text-xl font-bold text-white">New Setlist</h2>
                                </div>
                                <p className="text-sm text-slate-400 mb-4">Name it, then add your songs one by one.</p>
                                <div className="flex items-center gap-1.5 text-primary font-bold text-xs tracking-widest uppercase group-hover:translate-x-1 transition-transform">
                                    <Plus size={14} /> Create <ArrowRight size={12} />
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            <h2 className="text-xl font-bold text-white mb-2 flex items-center gap-2">
                                <Users className="text-secondary" /> Join by ID
                            </h2>
